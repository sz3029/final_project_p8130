...
```{r backward_adj}
crime_df %>%
  filter(row(crime_df) == 6 | row(crime_df) == 123)
...
```{r}
model_1 = lm(crm_1000 ~ totalinc + poverty + hsgrad + beds_density + docs_density + region + pop_density + docs_density*totalinc + docs_density*beds_density, data = crime_after)
